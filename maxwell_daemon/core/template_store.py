    ...
    def _load_builtins(self) -> None:
        ...
        builtins = [
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Read the git log of {{ repo }} since {{ previous_tag }} and "
                    "append a new section to CHANGELOG.md following the "
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Review {{ file_path }}. Add PEP 257 compliant docstrings to all "
                    "functions and classes that are missing them. Do not change any logic."
                ),
                parameters=[
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Review the codebase in {{ repo }} and update the README.md to "
                    "ensure the feature list, setup instructions, and usage examples "
                    "are accurate."
                ),
                parameters=[TemplateParameter(name="repo", type=ParameterType.REPO, required=True)],
                tags=["docs"],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    ...
                    "Check for logic errors, security issues, performance bottlenecks, "
                    "and style violations. Produce a consolidated review."
                ),
                parameters=[
                    TemplateParameter(name="repo", type=ParameterType.REPO, required=True),
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    ...
                    "{{ file_path }}. Place the tests in {{ test_path }} and aim "
                    "for high coverage of edge cases."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True),
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Run the test suite in {{ target_dir }}. Identify any failing "
                    "tests, diagnose the root cause (whether the code is broken or "
                    "the test is outdated), and apply the necessary fixes."
                ),
                parameters=[
                    TemplateParameter(name="target_dir", type=ParameterType.STRING, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Refactor {{ file_path }} to use Python's async/await where "
                    "appropriate (e.g., I/O bound operations). Ensure backwards "
                    "compatibility or update callers accordingly."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Add strict Python type hints to all function signatures and "
                    "complex variables in {{ file_path }}. Run mypy to ensure there "
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Perform a security audit on {{ target_dir }}. Look for SQL "
                    "injection, hardcoded secrets, XSS vulnerabilities, and unsafe "
                    "file I/O. Provide a remediation report."
                ),
                parameters=[
                    TemplateParameter(name="target_dir", type=ParameterType.STRING, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Check the dependencies defined in {{ file_path }} for newer "
                    "secure versions. Update the file and ensure that `pip install` "
                    "works without resolution conflicts."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    ...
                    "Suggest indexes, N+1 query fixes, or query rewrites to improve "
                    "execution speed."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True)
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Create a new Typer CLI command in {{ file_path }} named "
                    "`{{ command_name }}`. It should accept options for `verbose` "
                    "and `dry-run`, and include basic docstrings."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True),
                    TemplateParameter(
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Add a new `{{ method }}` route at `{{ route_path }}` to the "
                    "FastAPI router in {{ file_path }}. Include a basic Pydantic "
